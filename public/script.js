async function getNewsDataFromServer(z) {
  await fetch(`http://localhost:3000/news?topic=${z}`)
    .then((response) => {
...
function addEvents(topics) {
  document.getElementById(topics).addEventListener("click", function (e) {
    ...
    getNewsDataFromServer(topics);
  });
}

document.addEventListener("DOMContentLoaded", function () {
  addEvents("Geopolitics");
  addEvents("Entertainment");
  addEvents("Stocks");
  addEvents("Business");
  addEvents("Sports");
  document.getElementById("searh").addEventListener("click", function (e) {
    e.preventDefault();

    const topics = document.getElementById("input").value;
    console.log(topics)
    const cardsPlace = document.getElementById("cardsPlace");
    while (cardsPlace.firstChild) {
      cardsPlace.removeChild(cardsPlace.firstChild);
